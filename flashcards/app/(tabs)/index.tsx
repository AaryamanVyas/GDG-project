...
export default function DecksScreen() {
  ...
  const closeModal = () => setShowKey(false);

  const handleSaveKey = () => {
    setOpenAIApiKey(keyInput.trim() || undefined);
    closeModal();
  };
    ...
    <View style={styles.container}>
      <View style={styles.headerRow}>
        ...
        <TouchableOpacity style={styles.apiBtn} onPress={() => setShowKey(true)}>
          <Text style={styles.apiBtnText}>
            {state.openaiApiKey ? 'API Key ✓' : 'Set API Key'}
          </Text>
        </TouchableOpacity>
      </View>

      <Text style={styles.coins}>Coins: {state.coins ?? 0}</Text>
      ...
      <Modal transparent visible={showKey} animationType="fade" onRequestClose={closeModal}>
        <View style={styles.modalWrap}>
          <View style={styles.modalCard}>
            ...
            <View style={styles.modalActions}>
              <TouchableOpacity style={[styles.addBtn, { flex: 1 }]} onPress={handleSaveKey}>
                <Text style={styles.addBtnText}>Save</Text>
              </TouchableOpacity>
              <TouchableOpacity
                style={[styles.apiBtn, { flex: 1, alignItems: 'center' }]}
                onPress={closeModal}
              >
